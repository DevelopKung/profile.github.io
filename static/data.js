export default {
  full_name: 'ATIWAT SUTTSAK',
  image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQS9QE32spMNwUUnA4RfpJqTLoNLIKC0NgXaw&usqp=CAU',
  nickname: 'TuMKunG Developer',
  position: 'Full Stack Developer',
  contact: [{
    title: 'Full Name',
    value: 'TuMKunG Developer',
    icon: 'fas fa-user'
  },
  {
    title: 'Email',
    value: 'atiwat.sut@outlook.co.th',
    icon: 'fas fa-envelope'
  },
  {
    title: 'Phone',
    value: '062-xxx-xxxx',
    icon: 'fas fa-phone-volume'
  },
  {
    title: 'Address',
    value: 'xx/xx ต.ในเมือง อ.เมือง จ.ขอนแก่น 40000',
    icon: 'fas fa-home'
  }],
  items: [{
    img: 'https://cdn.pixabay.com/photo/2016/11/30/17/10/web-1873373_1280.png',
    text: 'WEB',
  },
  {
    img: 'https://play-lh.googleusercontent.com/PCpXdqvUWfCW1mXhH1Y_98yBpgsWxuTSTofy3NGMo9yBTATDyzVkqU580bfSln50bFU',
    text: 'GITHUB',
  },
  {
    img: 'https://gitlab.com/uploads/-/system/project/avatar/278964/project_avatar.png',
    text: 'GITLAB',
  },
  {
    img: 'https://www.facebook.com/images/fb_icon_325x325.png',
    text: 'FACEBOOK',
  }],
  skill: [{
    img: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQpngGRjYX1ca7qAADU3K6eGLj7ShQE3L2otdzfryl_Y9Ht2QRoQKYQbsXd36XIxMbYOw0&usqp=CAU',
    text: 'HTML',
    power: '80'
  },
  {
    img: 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1200px-CSS3_logo_and_wordmark.svg.png',
    text: 'CSS',
    power: '80'
  },
  {
    img: 'data:image/png;base64,',
    text: 'CSS',
    power: '80'
  },
  {
    img: 'https://avatars.githubusercontent.com/u/6128107?s=280&v=4',
    text: 'Vue.js',
    power: '70'
  },
  {
    img: 'https://www.datocms-assets.com/205/1667561117-nuxt.svg?auto=format&fit=max&w=1200',
    text: 'Nuxt.js',
    power: '80'
  },
  {
    img: 'https://avatars.githubusercontent.com/u/22138497?s=200&v=4',
    text: 'Vuetify',
    power: '80'
  }],
}